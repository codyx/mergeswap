...
async function work(request: Request, env: Env, ctx: ExecutionContext) {
  ...
  if (chainConfig == null) {
    throw new Error(
      `no config defined for chainHandle '${params.chainHandle}'`
    );
  }
  ...
  const confirmedTipBlockNumber = Math.max(
